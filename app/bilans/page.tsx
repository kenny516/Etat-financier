...
import { FinancialDataSelectionForm } from "@/components/financial-data-bilan";
...
export default function BilansPage() {
    ...
    const companies = ["Example Corp", "Another Inc", "Test LLC"]; // Replace with actual company list
    const years = [2024, 2023, 2022, 2021, 2020]; // Replace with actual year list

    const handleFormSubmit = async (data: { company: string; year: string }) => {
        setLoading(true);
        setError(null);
        try {
            const response = await axios.get<FinancialData>('/api/financial-data', {
